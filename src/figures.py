...
def fig3_state_inflation(df, outdir="figures", criterion="bic"):
    ...
    for i, (a, mode) in enumerate(zip(axes[0], modes)):
        sub = s[s["mode"] == mode]
        ...
        # Only annotate "true k" on the first panel and anchor it bottom-left,
        # so it never competes with the legend (placed top-left of the LAST
        # panel below). Previously this was drawn on every panel at the same
        # top-left corner the legend used, causing a collision in whichever
        # panel got both.
        if i == 0:
            a.text(0.02, 0.04, f"true k = {k_true}", transform=a.transAxes,
    ...
    axes[0][-1].legend(title="projection", loc="upper left",
                       framealpha=0.9)
