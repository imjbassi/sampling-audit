...
\textbf{A practical recommendation, not just a diagnosis.} Five model-selection
...
\textbf{Alanine dipeptide} (all-atom, 3 basins). Two independent 100 ns Langevin
dynamics simulations in implicit solvent (OpenMM \cite{eastman2017}, amber14 \cite{maier2015} with
GBn2 \cite{nguyen2013}, 2 fs timestep, 1 ps save interval, 100,000 saved frames each). Ground truth is the standard three-region
...
\textbf{VAE} \cite{kingma2014,hernandez2018} (fixed architecture, trained for a fixed number of gradient
steps rather than a fixed number of epochs, so that larger datasets are not
silently given more optimization). t-SNE \cite{vandermaaten2008} is reported only in an appendix
figure: its cluster count is governed substantially by its perplexity
hyperparameter rather than by data structure \cite{wattenberg2016}, so treating its output as a
...
\section{The effect is not reducible to one selection criterion}
...
\textbf{AIC is saturated, not budget-driven.} AIC reaches the search ceiling in
...
\textbf{ICL never saturates, but it is not accurate.} ICL is the only criterion that
...
\caption{The same trend under all five selection criteria. BIC inflates monotonically; AIC is saturated at every budget rather than trending; ICL alone stays in an interpretable range; silhouette and elbow gap are insensitive to budget and to landscape alike.}
\label{fig:criteria}
\end{figure*}
...
\section{The effect is not explained by incomplete exploration}
...
\texttt{short} and \texttt{subsample} are statistically indistinguishable in every cell (Table~\ref{tab:coverage}):

\begin{table*}[t]
\centering
\small
\caption{The coverage-matched control. BIC budget correlations under genuinely short trajectories and under uniform thinning of one long reference trajectory. Intervals overlap in all six comparisons, so holding landscape coverage fixed does not remove the effect.}
\label{tab:coverage}
\begin{tabular}{lccc}
\toprule
\textbf{system} & \textbf{method} & \textbf{short} & \textbf{subsample} \\
\midrule
...
happened to visit. (Figure~\ref{fig:coverage})
...
ARI = 1.0 trivially. Such conditions occur only in \texttt{short} mode at small
...
\centering
\small
\caption{TICA lag ablation on the Prinz potential: oracle-$k$ recovery as a function of sampling budget and lag (10 seeds, subsample mode, 300 conditions). Recovery falls monotonically as lag grows at fixed budget, and the budget needed to reach any given level rises with lag.}
...
fewer usable lagged pairs (n\_frames - lag) producing a less stable time-lagged
...
\centering
\small
\caption{All nine method--system pairs. Every pair declines from its peak to the largest budget tested; the size of the decline tracks how much budget remains after TICA's warm-up completes, and is otherwise uniformly large for PCA and VAE.}
\label{tab:ninepairs}
\begin{tabular}{lccccc}
\toprule
...
\textbf{TICA's decline tracks how much budget remains after its warm-up, not the
...
completes (Table~\ref{tab:warmup}):

\begin{table*}[t]
\centering
\small
\caption{Why TICA's decline varies by system. Warm-up completion is read from the oracle-$k$ column; what remains of the sweep after it determines how much ordinary selection-driven degradation can occur.}
\label{tab:warmup}
\begin{tabular}{lccc}
\toprule
\textbf{system} & \textbf{warm-up completes at} & \textbf{doublings remaining in sweep} & \textbf{decline} \\
\midrule
Prinz & not before n=32,000 (end of range) & 0 & 12\% \\
Alanine dipeptide & n=16,000 & 2 & 37\% \\
Müller-Brown & n$\approx$1,000-4,000 & 3-5 & 53\% \\
\bottomrule
\end{tabular}
\end{table*}
...
\textbf{Why alanine's warm-up completes so late, and a caution it implies.} That
...
to the \texttt{subsample} condition. TICA's lag is specified in frames of the data it
...
\centering
\small
\caption{TICA under coverage-matched subsampling on alanine dipeptide. Thinning to $n$ frames from a 100,000-frame reference imposes a stride of $100{,}000/n$, so the effective physical lag shrinks as the budget grows. Projection quality tracks the effective lag, not the budget.}
\label{tab:efflag}
\begin{tabular}{lccccc}
\toprule
...
\caption{Thinning a trajectory rescales TICA's lag. (a) Warm-up against sampling budget for both modes. (b) The same points against the effective lag $\tau_{\mathrm{eff}} = L\,(N/n)\,\Delta t$: every subsample point above the dihedral transition timescale sits near 0.25 and everything below it jumps to roughly 0.95, so lag alone accounts for that curve. The \texttt{short} points sit at a fixed 10~ps and still span 0.15 to 0.96, so their spread is sample size alone.}
\label{fig:efflag}
\end{figure*}
...
\textbf{Simulation.} A single 100 ns trajectory of alanine dipeptide (ACE-ALA-NME)
...
this trajectory: \texttt{short} budgets as contiguous leading blocks, \texttt{subsample}
...
psi) dihedral angles into the standard three-region partition (C7eq, alpha\_R,
alpha\_L/C7ax), a chemical definition independent of any method under audit.
...
\caption{Alanine dipeptide ground truth, from the seed-1 trajectory: (a) Ramachandran density, (b) the standard three-region partition that supplies the basin labels, (c) basin occupancy. The partition is a chemical definition and is independent of every method under audit. The rarest basin holds 5.3\% of frames, which is why sampling budget bites hardest on this system.}
\label{fig:rama}
\end{figure*}
...
\centering
\small
\caption{Mean number of the three ground-truth basins visited by contiguous leading blocks of the 100~ns trajectory. Under coverage-matched subsampling all three are visited at every budget.}
\label{tab:basins}
...
\textbf{Replication of the main effect.} BIC-selected state count increases with
...
\centering
\small
\caption{Two independent 100~ns alanine dipeptide seeds, compared over the budgets they share. Budget inflation, the method ranking and basin coverage all replicate. TICA's relative decline does not: its warm-up completes one budget doubling later on seed 1, leaving no room for selection-driven degradation within the tested range, exactly as the doublings-remaining account predicts.}
\label{tab:crossseed}
\begin{tabular}{lcc}
\toprule
...
\item \textbf{Do not report a state count from a single budget as a stable property of the system.} Where feasible, report how the count changes with budget (as in Table~\ref{tab:ninepairs}), or explicitly justify why the budget used is believed to be past the point where this matters---which, per point 3, requires checking rather than assuming.
...
\item \textbf{Prefer ICL over BIC or AIC for this purpose, with the caveat that ICL is not thereby accurate, only less prone to runaway inflation.} ICL never reached the search ceiling in any of the twelve (system x mode x method) conditions tested here, where BIC did so in up to 44\% of conditions and AIC in up to 84\%. ICL's own reported count should still be treated as an estimate to sanity-check, not a final answer.
\item \textbf{Where TICA is used, check for evidence of a warm-up regime before trusting its output at a given budget.} TICA's failure mode at small budgets (near-zero recovery, not merely inaccurate recovery) is distinct from PCA's and VAE's and is not fixed by any of the five selection criteria tested; it requires enough usable lagged pairs (sampling budget minus lag) at the chosen lag to produce a stable estimate. This is not only a plausible diagnostic but a measured one: the lag ablation in Section 6 shows recovery at fixed budget falling monotonically as lag increases, and the budget needed to reach any given recovery level rising correspondingly with lag. A practical check even without ground truth is whether the projection changes substantially under a moderate change in lag; if it does, the estimate at the current lag and budget should not yet be trusted.
\end{enumerate}
...
\textbf{TICA's warm-up completion budget is seed-dependent, so the size of its
...
\textbf{TICA's lag parameter was varied on one system only.} The warm-up mechanism
