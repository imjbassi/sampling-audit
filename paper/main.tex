...
\subsection{Sampling Budget as a Confound in Dimensionality-Reduction Analyses of Molecular Simulation}
...
\caption{Ground truth for the M\``uller--Brown potential: (a) the energy surface with its three minima, (b) basin assignment by steepest-descent quenching, (c) one sampled trajectory coloured by true basin. Basin labels never come from a fitted model.}
\label{fig:landscape}
\end{figure*}
...
\caption{Embeddings of the M\''uller--Brown system across projection method (rows) and sampling budget (columns), coloured by true basin. The underlying landscape is identical in every panel; only the number of saved frames differs.}
...
- \textbf{short}: a trajectory run to produce exactly the target number of saved
...
- \textbf{subsample}: one long reference trajectory per seed, uniformly thinned to
...
Müller-Brown & n\$\textbackslash{}approx\$1,000-4,000 & 3-5 & 53\% \\
\bottomrule
\end{tabular}
\end{table}
...
1. \textbf{Report the sampling budget alongside any claimed metastable state count.}
...
2. **Do not report a state count from a single budget as a stable property of
   ...
   (as in Table~\ref{tab:ninepairs}), or explicitly justify why the budget used is
